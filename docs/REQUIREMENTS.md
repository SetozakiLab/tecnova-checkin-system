...
- **活動ログ記入機能 (Activity Log)** [✖ 新規追加中]:
  - 対象: 現在チェックイン中のゲストのみ。
  - 30 分単位タイムスロット（JST, 分:00 / :30）で 1 ゲスト 1 レコード。サーバー側で現在時刻から切り捨て正規化。
  - カテゴリ（Enum `ActivityCategory`）固定選択 + 説明 (1–100 文字, プレーンテキスト)。
  - カテゴリ一覧: VR（HMD）, ドローン, 3D プリンタ, ペッパー, レゴ, mBot2, little Bits, MESH, toio, マインクラフト, Unity, Blender, DaVinci Resolve, その他(OTHER)。
  - OTHER 選択時も同一 `description` フィールドで補足（別フィールド無し）。
  - MANAGER/SUPER 共に作成・更新可。削除は SUPER のみ。
  - 過去スロットへの遡及入力許可（MVP では在席実績との厳密突合は行わず将来警告/制約）。
  - MVP では単一入力画面(A006)のみ（一覧/検索/集計は将来拡張）。
...
- **活動ログ一覧/検索/分析**: (計画) 日付/ゲスト/カテゴリ/学年フィルタ + アクティビティ集計（カテゴリ別時間・ヒートマップ）。
- **活動ログ監査履歴**: (計画) 編集/削除時の before/after を監査テーブルへ。
- **活動ログエクスポート**: (計画) CSV / TSV / 日次・月次集計。
...
- **`ActivityLog` (活動ログ / 30 分枠)**
  - `id`: UUID (主キー)
  - `guestId`: UUID (Guest 外部キー, ON DELETE CASCADE)
  - `timeslotStart`: DateTime (JST 基準で分=00 / 30 のみ許容)
  - `category`: ActivityCategory (Enum)
  - `description`: String (1–100 文字, プレーンテキスト)
  - `createdByUserId`: UUID (User 外部キー)
  - `createdAt`: DateTime
  - `updatedAt`: DateTime
...
- **UI/UX**:
  ...
  - **活動ログ入力 UI 指針**: 現在時刻を直前の 00/30 に正規化したスロットを初期選択。カテゴリセレクト + 説明テキスト 1 行（残文字カウンタ）。OTHER 選択時も同一説明欄利用。編集は同一フォームで再送信。削除ボタンは SUPER のみ表示。
