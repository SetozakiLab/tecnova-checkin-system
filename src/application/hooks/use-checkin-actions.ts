...
import type { CheckinRecordEntity } from "@/domain/entities/checkin-record";
import { useCheckinManagement } from "@/infrastructure/container";

interface UseCheckinActionsReturn {
  loading: boolean;
  error: string;
  checkinGuest: (guestId: string) => Promise<{
    success: boolean;
    record?: CheckinRecordEntity;
    error?: string;
  }>;
  checkoutGuest: (guestId: string) => Promise<{
    success: boolean;
    record?: CheckinRecordEntity;
    error?: string;
  }>;
  checkoutByRecordId: (recordId: string) => Promise<{
    success: boolean;
    record?: CheckinRecordEntity;
    error?: string;
  }>;
...
export function useCheckinActions(): UseCheckinActionsReturn {
  ...
  const checkoutGuest = useCallback(
    async (guestId: string) => {
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "チェックアウトに失敗しました";
        setError(errorMessage);
        return { success: false, error: errorMessage };
      } finally {
        setLoading(false);
  ...
  const checkoutByRecordId = useCallback(
