...
import type { GuestWithStatus } from "@/domain/entities/guest";
import type {
  CreateGuestParams,
  GuestSearchParams,
  UpdateGuestParams,
...
export function useGuestManagement(): UseGuestManagementReturn {
  ...
  const searchGuests = useCallback(
    async (params: GuestSearchParams) => {
      try {
        setLoading(true);
        setError("");
        ...
        setError(
          err instanceof Error ? err.message : "ゲストの検索に失敗しました",
        );
      } finally {
        setLoading(false);
      }
  ...
  const createGuest = useCallback(
    async (params: CreateGuestParams) => {
      try {
        setError("");
        ...
        const errorMessage =
          err instanceof Error ? err.message : "ゲストの作成に失敗しました";
  ...
  const updateGuest = useCallback(
    async (id: string, params: UpdateGuestParams) => {
      try {
        setError("");
        ...
        const errorMessage =
          err instanceof Error ? err.message : "ゲストの更新に失敗しました";
        setError(errorMessage);
        return { success: false, error: errorMessage };
      }
    },
  ...
  const deleteGuest = useCallback(
    async (id: string) => {
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "ゲストの削除に失敗しました";
        setError(errorMessage);
        return { success: false, error: errorMessage };
      }
    },
  ...
  const findGuestsByName = useCallback(
    async (name: string): Promise<GuestWithStatus[]> => {
      try {
        setError("");
        ...
        const guestsWithStatus: GuestWithStatus[] = await Promise.all(
          foundGuests.map(async (guest) => {
            const guestWithStatus = await guestManagement.getGuestWithStatus(
              guest.id,
            );
            return (
              guestWithStatus || {
